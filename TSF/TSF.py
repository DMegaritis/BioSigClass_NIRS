...
class TSF_Classifier:
    """
    This class trains a Time Series Forest (TSF) classifier using group k-fold cross-validation.
    ...
    def __init__(self, features, target, groups, n_splits=5, n_estimators=100):
    ...
    def train(self):
        """
        Train the TSF model using group k-fold cross-validation and evaluate using various metrics.
        ...
        for train_index, test_index in gkf.split(self.features, self.target, self.groups):
            ...
            tsf = TimeSeriesForestClassifier(n_estimators=self.n_estimators)
            tsf.fit(X_train, y_train)

            # TODO: check the outputs if they are 0 or 1 or 0-1, check if they work with below sklearn functions
            y_pred = tsf.predict(X_test)
            y_prob = tsf.predict_proba(X_test)[:, 1]
            ...
            y_train_prob = tsf.predict_proba(X_train)[:, 1]
        ...
        plt.title('ROC Curve for model evaluation (TSF)')
        ...
        plt.xlabel('Specificity')
        plt.ylabel('Sensitivity')
        plt.title('ROC Curve for training set (TSF)')
        plt.legend(loc='lower right')
        plt.show()
        ...
        final_tsf = TimeSeriesForestClassifier(n_estimators=self.n_estimators)

        final_tsf.fit(self.features, self.target)

        with open('pre_trained_tsf.pkl', 'wb') as file:
            pickle.dump(final_tsf, file)
